...
ui_file_path = os.path.join(script_dir, 'face.ui')

# Load the UI file
ui, _ = loadUiType(ui_file_path)
...
class MainApp(QMainWindow, ui):
    ...
    def start_training(self):
        ...
        if not os.path.isdir(path):
            os.makedirs(path)  # Creates all intermediate directories
        ...
        while True:
            ...
            for (x, y, w, h) in faces:
                ...
                if prediction[1] < 800:
                    ...
                    try:
                        con = sqlite3.connect("face-reco.db")
                        cursor = con.execute("SELECT * FROM attendance WHERE name = ? AND attendancedate = ?", (str(names[prediction[0]]), str(date.today())))
                        result = cursor.fetchall()
                        if result:
                            available = True
                        if not available:
                            con.execute("INSERT INTO attendance VALUES(?, ?, ?)", (attendanceid, str(names[prediction[0]]), str(date.today())))
                            con.commit()   
                    ...
                    if cnt > 100:
                        ...
                        cnt = 0
            cv2.imshow("Face Recognition - Press 'a' to exit", im)
            key = cv2.waitKey(10)
            if key == ord("a"):
                break
        webcam.release()
